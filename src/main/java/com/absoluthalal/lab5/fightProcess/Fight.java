package com.absoluthalal.lab5.fightProcess;


import com.absoluthalal.lab5.Result;
import com.absoluthalal.lab5.enemies.ShaoKahn;
import com.absoluthalal.lab5.enemyFabrics.EnemyFabric;
...
public class Fight {
    ...
    private int locationForImage;



    public void setCountOfAllLocation(int countOfAllLocation) {
        this.countOfAllLocation = countOfAllLocation;
    }

    private int countOfAllLocation;
    ...
    public int getCurrentLocation() {
        if(enemy instanceof ShaoKahn && currentLocation!=1){
        return currentLocation-1;}
        else if(enemy instanceof ShaoKahn && currentLocation==1){
    ...
    public void Hit(Player human, Player enemy, int a, JLabel label,
        ...
        if (human.getHealth() <= 0 | enemy.getHealth() <= 0) {
            if (currentLocation == countOfAllLocation+1 || human.getHealth()<=0) {
    ...
    public Player NewRound(Player human, JLabel label, JProgressBar pr1,
        ...
        System.out.println(roundsInLocation);
        if (currentRound==roundsInLocation-1) {
            enemy = action.ChooseBoss(label, label2, text, label3, human.getLevel(), human);
        } else if (currentRound < roundsInLocation-1) {
        ...
        action.HP(enemy, pr2);
        this.sign=true;
        if(currentRound == roundsInLocation) {
    ...
    public void prepareLocationAndRounds() {
        if (currentLocation <= countOfAllLocation) {
            currentLocation++;
            if(Math.random()<0.5) {
                roundsInLocation = 2;
            }
